...
destino_input = st.text_input("Escribe el número exacto del destino (ej. 58)").strip()

if destino_input:
    if 'df' not in locals():
        st.warning("⚠️ No hay datos cargados todavía.")
    else:
        resultados = df[df['Destino'].astype(str) == destino_input]
        if resultados.empty:
            st.warning("No se encontraron pedidos para ese destino exacto.")
        else:
